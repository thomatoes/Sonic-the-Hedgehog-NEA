...
class Game:
    def __init__(self):
        ...
        #Add [val, [x,y,w,h], object image.png/jpeg 
        # [x,y,w,h] --> x pos, y pos, width of rec, height of rect
        # first val , eg 0.25 - multiplier to make it scroll slower
    ...
    def load_level(self):
        ...
        #self.player1 = PhysicsEntity('player',(50,50),(32,64))
    ...
    def run(self):
        running = True
        num_repeats = math.ceil((self.game_screen.get_width() / self.bg.get_width()) + 10)
        while running:
            game_over = False
            ...
            if self.end_game:
                ...
                if self.end_frames == 60:
                    self.player.control_lock(15000)
            ...
            for chao in self.level_map.chao_sprites:
                ...
                #self.screen.blit(chao.image, chao_pos)

                if self.chao_interact:
                    if self.player.rect.x < 700:
                        self.game_screen.blit(self.assets['op'],(256,230))
                    else:
                        self.game_screen.blit(self.assets['so'],(400,150))
                ...
                if projectile[2] > 240:
                    ...
                    if self.player.rect.collidepoint(projectile[0]):
                        ...
                        for i in range(30):
                            ...
                            self.sparks.append(Spark(self.player.rect.center,angle,2+random.random())) #player blow up
            ...
            for ring in self.rings.copy():
                pos = (self.camera_group.offset.x, self.camera_group.offset.y)
                kill = ring.update(self.level_map)
            ...
            for sprite in self.level_map.goalpost_sprite:
                ...
                #self.level_map.goalpost_sprite.update()
            ...
            if not self.player.is_dead:
                for i in range(self.substep_val):
                    ...
                    self.player.detect_wall(self.level_map)
            ...
            # Call the sprite-entity collision methods
            if not self.player.is_dead:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and not self.player.lock_controls:
                    ...
                    if event.key == pygame.K_o:
                        ...
                        if not self.player.is_jumping and self.player.is_grounded and not self.player.is_charging_spindash:
                            ...
                            #Different animation state anyway
                            self.player.jump()
